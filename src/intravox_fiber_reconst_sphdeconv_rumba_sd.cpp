...
namespace fiber {
   ...
   arma::mat intravox_fiber_reconst_sphdeconv_rumba_sd(const arma::mat & Signal,
                                                   ...
                                                   int Niter)
    {
        ...
        // sigma0 = 1/15;
        double sigma0 = 1.0/15.0;
        
        // sigma2 = sigma0^2;
        double sigma2 = std::pow(sigma0,2);
        
        //N = Dim(1);
        int N = Signal.n_rows;
 
        // Reblurred_S = (Signal.*Reblurred)./sigma2;
        mat Reblurred_S(Signal.n_rows, Signal.n_cols);
        ...
        mat fODFi;
        mat Ratio;
        mat RL_factor(KernelT.n_rows,Reblurred.n_cols);
 ...
 */
            ...
            */
            ...
            for (int i = 0; i < Signal.n_rows; ++i) {
                for (int j = 0; j < Signal.n_cols; ++j) {
                    sum += (std::pow(Signal(i,j),2) + std::pow(Reblurred(i,j),2)/2 - (sigma2*Reblurred_S(i,j)) * Ratio(i,j));
                }
            }
            ...
            sigma2 = std::min(std::pow(1.0/10.0,2),std::max(sigma2_i,std::pow(1.0/50.0,2)));
