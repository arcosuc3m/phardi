...
namespace pfiber {
    ...
    void Multi_IntraVox_Fiber_Reconstruction(std::string diffSignalfilename,
        ...
        auto Ngrad = diffGrads.n_rows;
        
        mat diffBvalsM;
        diffBvalsM.load(diffBvalsfilename, arma::raw_ascii);
        
        vec diffBvals (diffBvalsM.n_elem);
        for (int i = 0; i < diffBvalsM.n_elem; ++i) diffBvals(i) = diffBvalsM(0,i);
        ...
        // ydiff = Vdiff(1).dim(2);
        // zdiff = Vdiff(1).dim(3);
        auto xdiff = regionDiff.GetSize()[0];
        auto ydiff = regionDiff.GetSize()[1];
        auto zdiff = regionDiff.GetSize()[2];

        auto Nd = V.n_rows;

        std::vector<cube> Vdiff(regionDiff.GetSize()[3]);
        Index4DType coord_4d;



        for (int w = 0; w < regionDiff.GetSize()[3]; ++w) {
                cube temp(regionDiff.GetSize()[0], regionDiff.GetSize()[1], regionDiff.GetSize()[2]);
                coord_4d[3]=w;
        ...
        cube Vmask(regionMask.GetSize()[0],regionMask.GetSize()[1],regionMask.GetSize()[2]);
        ...
        mat Kernel (V.n_rows + 2, diffGrads.n_rows);
        ...
        create_Kernel_for_rumba(V,diffGrads,diffBvals,opts.rumba_sd.lambda1, opts.rumba_sd.lambda2, opts.rumba_sd.lambda_csf, opts.rumba_sd.lambda_gm, Kernel);

        mat slicevf_CSF(xdiff,ydiff,fill::zeros);
        mat slicevf_GM(xdiff,ydiff,fill::zeros);

        cube globODFslice (xdiff,ydiff,Nd,fill::zeros);
        ...
        for (int slice = 0; slice < zdiff; ++slice) {
           ...
           if (accu(Imask) != 0) {
                   ...
                   for (int graddir = 0; graddir < Ngrad; ++graddir) {
                       ...
                       Idiff.slice(graddir) = Vdiff[graddir].slice(slice) % Imask; // product-wise multiplication
                   }

                   //inda = find(squeeze(Idiff(:,:,1))>0);
                   uvec inda_vec = find(Idiff.slice(0));
                   mat inda = conv_to< mat >::from(inda_vec);

                   //totalNvoxels = prod(size(Imask));
                   auto  totalNvoxels = Imask.n_elem;
  
                   //allIndexes = repmat(inda(:)',[Ngrad 1]); 
                   mat allIndixes = repmat(inda.t(),Ngrad,1);
                   ...
                   mat ind =  allIndixes + (totalNvoxels *  repmat(linspace<mat>(0, Ngrad-1,Ngrad),1,inda.n_elem));
                   mat diffSignal(Ngrad,inda.n_elem);
                   for (int i = 0; i < Ngrad; ++i) {
                       for (int j = 0; j < inda.n_elem; ++j)
		       ...
		       case RUMBA_SD:
			     ...
			     BOOST_LOG_TRIVIAL(info) << "calling intravox_fiber_reconst_sphdeconv_rumba_sd";
                             ...
                             ODF = intravox_fiber_reconst_sphdeconv_rumba_sd(diffSignal, Kernel, fODF0, opts.rumba_sd.Niter);           
		        ...
		        break;
                   ...
                   mat allIndexesODF = repmat(inda.t(),ODF.n_rows,1);

                   // ODFindexes = allIndexesODF + totalNvoxels*repmat([0:size(ODF,1)-1]',[1 length(inda) ]); % Indexes in 4D
                   mat ODFindexes = allIndexesODF + totalNvoxels * repmat(linspace<mat>(0, ODF.n_rows - 1, ODF.n_rows ),1,inda.n_elem);  
